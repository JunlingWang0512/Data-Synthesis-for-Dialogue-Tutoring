...
def check_if_string_is_number(input_string):
    try:
        float(input_string)
        return True
    except ValueError:
...
def is_informative(sentence, THRESHOLD):
    ...
    if num_stopwords / len(doc) > STOPWORDS_THRESHOLD:
        return False
    if check_if_string_is_number(sentence):
        return False
    return True
            ...
            if(len(sentences) == 1):
                ...
                for idx, sentence in enumerate(sentences):
                   ...
                   #需要修改
                    ...
                    prediction = prediction.replace("Teacher: ", "")
                    prediction = prediction.replace("Teacher:", "")
                ...
                print(output_data)
                prediction_output_file = '/cluster/scratch/wangjun/GPT_results/GPT_3.5/business_search_output.json'
