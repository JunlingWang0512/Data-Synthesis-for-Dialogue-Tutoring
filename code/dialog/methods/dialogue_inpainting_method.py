...
class DialogueInpaintingMethod(Method):
    def __init__(self, model_args, data_args, config, tokenizer: PreTrainedTokenizerFast):
        super().__init__(model_args, data_args, config, tokenizer)
    
    # need a prepreprocess function
    
    def preprocess_features(self, features):
        # Iterate through the dialogues in the batch
        for dialogue in features["turns"]:
            # Randomly choose a turn to mask
            masked_turn_index = random.choice(range(len(dialogue)))
            
            # Tokenize the context and the target turn
            context_tokens = []
            for i, turn in enumerate(dialogue):
                if i != masked_turn_index:
                    # Tokenize and add the speaker token before the turn text
                    speaker_token = self.model_args.user_token if turn["role"] == "user" else self.model_args.system_token
                    context_tokens.extend(self.tokenizer(speaker_token + turn["text"])["input_ids"])
            
            target_turn = dialogue[masked_turn_index]
            target_speaker_token = self.model_args.user_token if target_turn["role"] == "user" else self.model_args.system_token
            target_tokens = self.tokenizer(target_speaker_token + target_turn["text"])["input_ids"]
            
            # Truncate and pad context and target tokens if necessary
            max_context_length = self.data_args.max_context_length
            max_target_length = self.data_args.max_target_length
            context_tokens = context_tokens[-max_context_length:]
            target_tokens = target_tokens[:max_target_length]
            
            # Save the preprocessed tokens
            dialogue["input_ids"] = context_tokens
            dialogue["labels"] = target_tokens
